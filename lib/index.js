...
async function getGistFiles(gistIdOrUrl) {
    ...
    if (!gistIdOrUrl.includes('://')) {
        ...
        if (parts.length === 3) {
            ...
            gistId = parts[0].length === 20 || parts[0].length === 32
                ? parts.join('/')
                : parts.slice(1).join('/');
...
function isGistId(str) {
    ...
    if (str.includes('/')) {
        ...
        return part.length === 40 ? false : (str.split('/')[0].length === 20 || str.split('/')[0].length === 32);
    }
    return str.length === 20 || str.length === 32;
}
...
async function writeGistFile(gistIdOrUrl, gistAlias, to, projName) {
    ...
    if (initFile) {
        ...
        for (const line of lines) {
            ...
            // Execute command
            console.log(cmd);
            const cmdArgs = replaceMyApp(cmd.substring(cmd.indexOf(' ') + 1), projName.replace(/\./g, '_'));
            const executable = cmd.split(' ')[0];
            
            try {
                execSync(`${executable} ${cmdArgs}`, { 
                    cwd: hostDir, 
                    stdio: 'inherit',
                    shell: true 
                });
